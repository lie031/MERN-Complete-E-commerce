...
import CarDrawer from '../layout/CarDrawer'
  ...
  return (
    <>
      <nav className='container mx-auto flex justify-between py-4 px-6'>
        ...
        {/* left - logo */}
        <div className=''>
          <Link to='/' className='text-2xl font-md'>ShopSphere</Link>
        </div>
        ...
        <div className='flex items-center space-x-4'>
          ...
          <button onClick={toggleDrawer} className='relative hover:text-black'>
            ...
            <span className='absolute -top-1 bg-red text-xs rounded-full px-2 py-0.5'>
              4
            </span>
          </button>
          {/* search */}
      ...
      <CarDrawer drawerIsOpen={drawerIsOpen} toggleDrawer={toggleDrawer} />
      ...
      <div className={`fixed top-0 left-0 w-3/4 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform  duration-300 z-50 ${navDrawerIsOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className='p-4 '>
          ...
          <nav className='space-y-4'>
            <Link to='/collections' onClick={toggleNavDrawer} className='block text-gray-600 hover:text-black'>Men</Link>
